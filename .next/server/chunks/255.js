exports.id = 255;
exports.ids = [255];
exports.modules = {

/***/ 444:
/***/ ((module) => {

// Exports
module.exports = {
	"root": "style_root__Ysqjy"
};


/***/ }),

/***/ 255:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony import */ var react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(997);
/* harmony import */ var react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var maxrects_packer__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(510);
/* harmony import */ var maxrects_packer__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(maxrects_packer__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _util__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(828);
/* harmony import */ var _style_module_scss__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(444);
/* harmony import */ var _style_module_scss__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_style_module_scss__WEBPACK_IMPORTED_MODULE_3__);
/* provided dependency */ var React = __webpack_require__(689);




/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (React.memo(function({ className , files , padding: padding1 , cropped , extruded , ...props }) {
    const [state, dispatch] = (0,_util__WEBPACK_IMPORTED_MODULE_2__/* .useReducer */ ._Y)({
        gap: 0
    });
    const root = React.useRef();
    const canvas = React.useRef();
    const { current: mut  } = React.useRef({
        timestamp: 0,
        packer: new maxrects_packer__WEBPACK_IMPORTED_MODULE_1__.MaxRectsPacker(2048, 2048)
    });
    /**
   * 对图片进行排列
   */ const layout = (sprites, padding = 0)=>{
        const { packer , ctx  } = mut;
        packer.reset();
        packer.padding = padding;
        packer.addArray(sprites.map((sprite)=>({
                sprite,
                width: sprite.width,
                height: sprite.height,
                x: 0,
                y: 0
            })
        ));
        let w = 0;
        let h = 0;
        for (const item of packer.rects){
            w = Math.max(item.x + item.width, w);
            h = Math.max(item.y + item.height, h);
        }
        canvas.current.width = w;
        canvas.current.height = h;
        canvas.current.style.width = `${w}px`;
        canvas.current.style.height = `${h}px`;
        for (const item1 of packer.rects){
            ctx.drawImage(item1.sprite.canvas, item1.x, item1.y);
        }
    };
    React.useEffect(()=>{
        if (!files.length) return;
        Promise.all(Array.from(files, (file)=>{
            return new Promise(async (resolve)=>{
                const img = new Image();
                img.dataset.name = file.name;
                img.src = await (0,_util__WEBPACK_IMPORTED_MODULE_2__/* .readFile */ .pJ)(file);
                img.onload = ()=>resolve(img)
                ;
            });
        })).then((imgs)=>{
            let x = 0;
            let y = 0;
            mut.ctx.clearRect(0, 0, canvas.current.offsetWidth, canvas.current.offsetHeight);
            layout(imgs.map((img)=>new Sprite({
                    img,
                    cropped,
                    extruded
                })
            ), padding1);
        });
    }, [
        files,
        cropped,
        extruded,
        padding1
    ]);
    (0,_util__WEBPACK_IMPORTED_MODULE_2__/* .useMount */ .b6)(()=>{
        mut.ctx = canvas.current.getContext('2d');
        root.current.addEventListener('contextmenu', onContextmenu);
        root.current.addEventListener('wheel', onWheel);
        return ()=>{
            root.current.removeEventListener('contextmenu', onContextmenu);
            root.current.removeEventListener('wheel', onWheel);
        };
    });
    const onContextmenu = (e)=>{
        e.preventDefault();
    };
    const onWheel = (e)=>{
        e.preventDefault();
        e.stopPropagation();
        console.log('ctrl', e.ctrlKey);
        const now = Date.now();
        mut.elapsed = now - mut.timestamp;
        if (mut.elapsed > 20) return;
        mut.timestamp = now - mut.elapsed % 20;
        // 无需放缩
        // if (!canvas.current.offsetWidth) return
        const { offsetWidth: w , offsetHeight: h  } = canvas.current;
        let scale = 0;
        if (e.ctrlKey) {
            scale -= e.deltaY * 0.01;
        }
        console.log(scale);
    // if (scale) {
    //   canvas.current.style.width = `${w * (0 + scale) | 0}px`
    //   canvas.current.style.height = `${h * (0 + scale) | 0}px`
    // }
    };
    return(/*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx("section", {
        ...props,
        ref: root,
        className: [
            (_style_module_scss__WEBPACK_IMPORTED_MODULE_3___default().root),
            className
        ].join(' ').trim(),
        children: /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx("canvas", {
            ref: canvas
        })
    }));
}));
class Sprite {
    get width() {
        return this.canvas.width;
    }
    get height() {
        return this.canvas.height;
    }
    constructor({ img , cropped , extruded  }){
        const canvas = this.canvas = document.createElement('canvas');
        canvas.width = img.width;
        canvas.height = img.height;
        this.ctx = canvas.getContext('2d');
        this.raw = img;
        this.name = img.dataset.name;
        if (cropped) this.crop();
        else this.ctx.drawImage(img, 0, 0);
        extruded && this.extrude(extruded);
    }
    extrude(size) {
        const { canvas , ctx , width , height  } = this;
        const data = ctx.getImageData(0, 0, width, height);
        const top = ctx.getImageData(0, 0, width, 1);
        const left = ctx.getImageData(0, 0, 1, height);
        const right = ctx.getImageData(width - 1, 0, 1, height);
        const bottom = ctx.getImageData(0, height - 1, width, 1);
        canvas.width += size * 2;
        canvas.height += size * 2;
        // top
        for(let i = 0; i < size; i++){
            ctx.putImageData(top, size, i);
        }
        // left
        for(let i1 = 0; i1 < size; i1++){
            ctx.putImageData(left, i1, size);
        }
        // right
        for(let i2 = canvas.width - 1; i2 >= canvas.width - size; i2--){
            ctx.putImageData(right, i2, size);
        }
        // bottom
        for(let i3 = canvas.height - 1; i3 >= canvas.height - size; i3--){
            ctx.putImageData(bottom, size, i3);
        }
        ctx.putImageData(data, size, size);
    }
    crop() {
        const { ctx , width , height , raw , canvas  } = this;
        ctx.drawImage(raw, 0, 0);
        // left
        let left = 0;
        loop: for(let x = 0; x < width; x++){
            const { data  } = ctx.getImageData(x, 0, 1, height);
            for(let i = 3; i < data.length; i += 4){
                if (data[i]) {
                    left = x;
                    break loop;
                }
            }
        }
        // right
        let right = width;
        loop: for(let x1 = width - 1; x1 > -1; x1--){
            const { data  } = ctx.getImageData(x1, 0, 1, height);
            for(let i = 3; i < data.length; i += 4){
                if (data[i]) {
                    right = x1 + 1;
                    break loop;
                }
            }
        }
        let top = 0;
        loop: for(let y = 0; y < height; y++){
            const { data  } = ctx.getImageData(0, y, width, 1);
            for(let i = 3; i < data.length; i += 4){
                if (data[i]) {
                    top = y;
                    break loop;
                }
            }
        }
        let bottom = height;
        loop: for(let y1 = height - 1; y1 > -1; y1--){
            const { data  } = ctx.getImageData(0, y1, width, 1);
            for(let i = 3; i < data.length; i += 4){
                if (data[i]) {
                    bottom = y1 + 1;
                    break loop;
                }
            }
        }
        const w = right - left;
        const h = bottom - top;
        canvas.width = w;
        canvas.height = h;
        ctx.drawImage(this.raw, left, top, w, h, 0, 0, w, h);
    }
}


/***/ })

};
;